...
test('AU primary navigation keeps the final public categories and booking destinations', () => {
  assert.deepEqual(AU_SECTION_IDS, ['whole-cakes', 'daily', 'something-fresh', 'chocolate', 'tea', 'goods'])
  assert.deepEqual(AU_NAVIGATION.map((item) => item.id), ['whole-cakes', 'daily', 'something-fresh', 'chocolate', 'tea', 'goods'])
  ...
  assert.deepEqual(
    ...
    ['Whole Cakes', 'Daily', 'Something Fresh', 'Chocolate', 'Cacao Tea', 'Goods'],
  )
...
test('English home copy establishes the Daegu to Sydney brand story', () => {
  ...
  assert.deepEqual(content.categories.map((category) => category.label), ['Verygood Chocolate', 'Cacao Tea', 'Goods'])
